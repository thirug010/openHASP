...
#ifndef HASP_FT6236T_TOUCH_DRIVER_H
#define HASP_FT6236T_TOUCH_DRIVER_H
...
// Read touch points
HASP_ATTRIBUTE_FAST_MEM bool FT6236U_getXY(int16_t* touchX, int16_t* touchY)
{
    if(ft6336u_touch->read_touch_number() == 1) {
        
            *touchX = TFT_WIDTH - ft6336u_touch->read_touch1_x();
            *touchY = TFT_HEIGHT - ft6336u_touch->read_touch1_y();
        LOG_INFO(TAG_DRVR, F("~~~~~~~~Touch X     : %d"), &touchY);
        LOG_INFO(TAG_DRVR, F("~~~~~~~~Touch Y     : %d"), &touchY);
        return true;
    } else {
        return false;
    }
...
IRAM_ATTR bool touch_read(lv_indev_drv_t* indev_driver, lv_indev_data_t* data)
{
    ...
    if(ft6336u_touch->read_touch_number() == 1) {
        ...
        data->point.x = TFT_WIDTH - ft6336u_touch->read_touch1_x();
        data->point.y = TFT_HEIGHT - ft6336u_touch->read_touch1_y();
        LOG_INFO(TAG_DRVR, F("~~~~~~~~Touch X     : %d"), data->point.x);
        LOG_INFO(TAG_DRVR, F("~~~~~~~~Touch Y     : %d"), data->point.y);
...
static inline void FT6236U_drv_init()
{
...
class TouchFt6236u : public BaseTouch {
  ...
  public:
    ...
    IRAM_ATTR bool read(lv_indev_drv_t* indev_driver, lv_indev_data_t* data)
    {
        int16_t touchX;
        int16_t touchY;
        if(FT6236U_getXY(&touchX, &touchY)) {
            if(hasp_sleep_state != HASP_SLEEP_OFF) hasp_update_sleep_state(); // update Idle
...
#warning Using FT6236
using dev::TouchFt6236u;
extern dev::TouchFt6236u haspTouch;

#endif // ARDUINO

#endif // HASP_FT6236T_TOUCH_DRIVER_H